...
document.getElementById('btn-show-qr').addEventListener('click', () => {
  ...
  // Student view URL = base app URL (no params needed - default is student form)
  const studentUrl = APP_URL;
  const qrUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=' + encodeURIComponent(studentUrl);
  container.innerHTML = `<img src="${qrUrl}" alt="QR" width="220" height="220" style="border-radius:8px;" onerror="this.parentElement.innerHTML='<p style=color:var(--red)>Failed to load QR. Check internet connection.</p>'">`;
