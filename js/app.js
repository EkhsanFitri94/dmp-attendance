...
// ─── Pure JS QR Code Generator (zero dependencies, offline-safe) ───
function generateQR(data) {
  // QR Version 5 (37x37 modules), Byte mode, EC level M
  const size = 37;
  ...
  // ── Encode data ──
  const bytes = []; let bits = '';
  bytes.push(0x40); // Byte mode
  const len = data.length;
  bytes.push((len>>8)&0xff, len&0xff);
  for (let i=0;i<len;i++) bytes.push(data.charCodeAt(i)&0xff);
  // Terminator
  bytes.push(0,0); while (bytes.length%2) bytes.push(0xec);
  // Pad to capacity (version 5 byte mode M: 134 bytes max)
  const pad = [0x11,0xec]; let pi=0;
  while (bytes.length < 134) { bytes.push(pad[pi]); pi=(pi+1)%2; }

  // ── Reed-Solomon generator polynomial (degree 26, GF 256) ──
  const GF_EXP = new Uint16Array(512), GF_LOG = new Uint16Array(256);
  let x=1; for (let i=0;i<255;i++) { GF_EXP[i]=x; GF_LOG[x]=i; x<<=1; if(x&256)x^=0x11d; }
  GF_EXP[255]=GF_EXP[0];
  ...
  function gfPolyMul(p1,p2) {
    ...
    for (let i=0;i<p1.length;i++) for (let j=0;j<p2.length;j++) r[i+j]^=gfMul(p1[i],p2[j]);
    return r;
  }
  // Generator poly for 26 EC codewords
  let gen = new Uint8Array([1]);
  for (let i=0;i<26;i++) gen = gfPolyMul(gen, new Uint8Array([1,GF_EXP[i]]));

  // Split into blocks (V5-M: 2 groups, 67 data + 13 ec each, total 134+26)
  // Simplified: single group approach
  const blockData = bytes.slice(0, 134);
  let remainder = new Uint8Array(blockData.length + 25);
  for (let i=0;i<blockData.length;i++) remainder[i]=blockData[i];
  for (let i=0;i<blockData.length;i++) {
    const f = remainder[0]; remainder=remainder.slice(1);
    if (f) for (let j=0;j<25;j++) remainder[j]^=gfMul(gen[j+1],f);
  }
  const ecw = remainder.slice(0, 26);
  const codewords = [...blockData, ...ecw];

  // ── Bitstream ──
  let bw = '';
  for (const cw of codewords) {
    for (let b=7;b>=0;b--) bw += (cw>>b)&1;
  }
    ...
    for (let dr=-1;dr<=7;dr++) {
      for (let dc=-1;dc<=7;dc++) {
        ...
        if (tr>=0&&tr<size&&tc>=0&&tc<size) {
          m[tr][tc]=(dr===-1||dr===7||dc===-1||dc===7||(dr>=2&&dr<=4&&dc>=2&&dc<=4))?1:0;
        }
      }
    }
  }
  ...
  // ── Format info bit (placeholder - simplified) ──
  // ── Alignment (V5: one at center pattern) ──
  for (let dr=-2;dr<=2;dr++) for (let dc=-2;dc<=2;dc++) {
    const tr=size-9+dr, tc=size-9+dc;
    if (tr>=0&&tr<size&&tc>=0&&tc<size) m[tr][tc]=(dr===-2||dr===2||dc===-2||dc===2||(dr===0&&dc===0))?1:0;
  }

  // ── Reserved areas ──

  // ── Place data bits (zigzag columns, bottom-up) ──
  let bi = 0;
  for (let c=size-1;c>=0;c-=2) {
    if (c===6) c=5; // skip timing column
    for (let row=0;row<size;row++) {
      const dir = (Math.floor(c/2)%2===0) ? 1 : -1;
      const r = dir===1 ? size-1-row : row;
      for (let dc=0;dc<=1;dc++) {
        const tc = c-dc;
        if (tc>=0&&tc<size&&r>=0&&r<size) {
          if (m[r][tc]===0) {
            if (bi < bw.length) {
              m[r][tc] = (bw[bi]==='1') ? 2 : 0;
              bi++;
  ...
  // ── Apply masks, pick best (mask 2 = row%3) ──
  function mask2(r,c) { return r%3===0 ? 1:0; }
  // Apply mask 2
...
document.getElementById('btn-show-qr').addEventListener('click', () => {
  ...
  const studentUrl = APP_URL;
  document.getElementById('qr-url-display').textContent = studentUrl;
  try {
    const dataUrl = generateQR(studentUrl);
    container.innerHTML = `<img src="${dataUrl}" alt="QR Code" width="220" height="220" style="border-radius:8px;">`;
  } catch(e) {
    container.innerHTML = '<p style="color:var(--red);">Failed to generate QR.</p>';
  }
});
